...
export function FailedRequestError(ex: unknown): APIError {
  ...
  let tryAsApiError = errorData as { message: string; successful: boolean };

  if (tryAsApiError && tryAsApiError.successful === false) {
    const api: ApiStatus = {
...
// const baseClient = createApiClient("http:/api.domain.com");
...
const apiClientRequest = {
  post: async (path: string, body: any = {}, customHeaders: AxiosRequestHeaders = {}): Promise<RemoteData<APIError, string | string[] | boolean | number | Date | undefined | null | any>> => {
  ...
  delete: async (path: string, body = {}, customHeaders: AxiosRequestHeaders = {}): Promise<RemoteData<APIError, string | string[] | boolean | number | Date | undefined | null | any>> => {
    const allQueryParams: TQueryParam[] = [];
    const queryParams = allQueryParams.map((param: TQueryParam) => `${param.key}=${param.value}`).join("&");
  ...
  token: store.getState().user.token?.accessToken || ''
};
