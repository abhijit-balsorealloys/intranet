...
export const FormButton: React.FC<IFormButton> = ({ wrapperClasses = "", className = "btn", type = "submit", disabled = false, loadingText = true, onClick = () => {}, text = "Save Changes", style = {}, title = "", element = null }) => {
	return (
		<>
			{wrapperClasses != "" ? (
				<div className={wrapperClasses}>
					<button title={title} className={classNames("form-btn", className)} style={style} type={type} disabled={disabled}>
						{disabled && <FaSpinner className="fa-spin mr-2" />} {element === null ? <>{!loadingText && disabled ? "" : text}</> : <>{element}</>}
					</button>
				</div>
			) : (
				<>
